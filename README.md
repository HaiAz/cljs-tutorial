...
* [`src/cljs_tutorial/`](src/cljs_tutorial/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs_tutorial/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `cljs-tutorial` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cljs_tutorial/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cljs-tutorial.example
  (:require [cljs-tutorial.config :as config])
